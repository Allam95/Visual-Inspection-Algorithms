    ...
    def unlabeledhistogram():
        ...
        for j in height_patches:
            ...
            for i in width_patches:
                ...
                try:
                    #features_unlabeled= mahotas.features.haralick(unlabeled).mean(0)
                    features_unlabeled = cv2.calcHist([unlabeled],[0],None,[256],[0,256])
                    y = [int(yes) for yes in features_unlabeled]
                    #print(y)
                    #print(list(features_unlabeled[0]))
                    #print(', '.join(features_unlabeled))
                    ...
                    if prediction == 1 :
                        ...
                        #cv2.imwrite(save_directory,inspect)
                        #print(save_directory)
                        #plt.imshow(inspect)
                        #plt.show()
                        #print(j ,j +50 , i , i+50)
                        image_coordinates = [j ,j +50 , i , i+50]
        ...
        for (startX, startY, endX, endY) in pick:
            cv2.rectangle(gear, (startX, startY), (endX, endY), (0, 255, 0), 2)                  
